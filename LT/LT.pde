...
void setup() 
{
  ...
  ltS = new Server(this, 10010); // This is the left text server, receiving data from clients directed at right
  ltMC = new Client(this, "127.0.0.1", 10000);        // connect to master
  ltVC = new Client(this, "127.0.0.1", 10015);        // connect to left visual server
  smooth(32);
  textFont(font0);
  fill(bg);
...
void draw() {
  ...
  Client client = ltS.available();        // define the client that's trying to send messages
  if (client != null) {
    if (client.available() > 0) {            // confirm this client has something to say
      input = client.readString();           // store the string to play with 
      ltMC.write(input);                      // relay to master server
      ltVC.write(input);                      // relay to left visual server
